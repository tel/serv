...
import Network.HTTP.Kinder.Header (HeaderDecode, HeaderName, Sing (SAllow, SContentType),
                                   SomeHeaderName, headerDecodeBS, headerEncodePair, headerName,
                                   parseHeaderName)
import Network.HTTP.Kinder.MediaType (AllMimeDecode, NegotiatedDecodeResult (..),
                                      negotiatedMimeDecode)
import Network.HTTP.Kinder.Query (QueryDecode (..), QueryKeyState (..))
...
class Contextual m where -- TODO: we can specify kind of m
